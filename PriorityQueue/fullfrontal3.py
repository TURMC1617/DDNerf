...
import sdl2
import sdl2.ext
from PQueue import populate_queue
from PQueue import pop
import serial
...
def processFrame(frame, depth):
	...
	if frontal != () and depth != []:
		...
		return  depth[center[1], center[0]]

	return None

def sweep(frame, depth, angle):
        
        sweep_bool = Queue.Queue()
        x = 0
        while(sweep_bool != 181):
            sweep_bool.put(x)
            x = x + 10
            
       if sweep_bool != 0:
               #calls processFrame
               pos = processFrame(frame, depth)
               tuple = (pos, angle)
               populate_queue(depth, angle)
               swoop_bool.get()
